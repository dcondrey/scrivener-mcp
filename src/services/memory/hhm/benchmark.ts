...
export async function quickBenchmark(dimensions: number = 10000): Promise<void> {
    ...
    console.log(`Avg Memorize: ${(memorizeTime / iterations).toFixed(2)}ms`);
    console.log(`Avg Query: ${(queryTime / iterations).toFixed(2)}ms`);
    console.log(`Ops/sec: ${((iterations * 2) / (totalTime / 1000)).toFixed(0)}`);
    console.log('-------------------------------------------\n');
}
